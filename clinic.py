...
if "lead_submitted" in st.session_state and st.session_state.lead_submitted:
    ...
    st.markdown(f"[📞 Call Now](tel:{nearest['contact']})")

    # WHATSAPP LINK
    whatsapp_number = nearest["contact"]

    message = f"Hello, I am {st.session_state.name}. I would like to consult at {nearest['name']}."

    whatsapp_url = f"https://wa.me/{whatsapp_number}?text={message}"

    st.markdown(f"[💬 Chat on WhatsApp]({whatsapp_url})")
